...
class DataHandler:
	...
	def LoadData(self):		
		...
		for i in range(len(self.behs)):
			...
			with open(path, 'rb') as fs:
				mat = pickle.load(fs)
				mat = 1*(mat != 0).astype(np.float32)
			trnUsrs = np.reshape(np.where(mat.sum(1)>0)[0], [-1])
			trnMats.append(mat)
			if i == len(self.behs)-1:
				
				rowidx = np.random.permutation(mat.shape[0])
				colidx = np.random.permutation(mat.shape[1])
				rowidxRandom = np.random.choice(rowidx, mat.nnz).astype(np.int32)
				colidxRandom = np.random.choice(colidx, mat.nnz).astype(np.int32)
				value = np.ones(mat.nnz).astype(np.float32)
				row, col = mat.shape
				randmat = csr_matrix((value, (rowidxRandom, colidxRandom)), shape=(row, col))
		self.randomMat	= randmat
		# test set
	...
	def sampleLargeGraph(self, pckUsrs, pckItms=None, sampDepth=2, sampNum=args.graphSampleN, preSamp=False):
		...
		def constructData(usrs, itms):
			...
			pckAdjs = []
			pckTpAdjs = []
			pckU = self.randomMat[usrs]
			tpPckI = transpose(pckU)[itms]
			pckRandTpAdj = tpPckI
			pckRandAdj = transpose(tpPckI)
			for i in range(len(adjs)):
				pckU = adjs[i][usrs]
				tpPckI = transpose(pckU)[itms]
				pckTpAdjs.append(tpPckI)
				pckAdjs.append(transpose(tpPckI))
			return pckAdjs, pckTpAdjs, usrs, itms, pckRandAdj, pckRandTpAdj
